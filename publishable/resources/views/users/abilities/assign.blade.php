...
@section('centurion-title')
    @lang('centurion::users.page_titles.assign_permissions')
@endsection
...
<form method="POST" action="{{ route('users.abilities.sync', $user->id) }}">
	...
	@component('centurion::components/errors')
		@slot('title')
			@lang('centurion::users.headings.assign_permissions_validation_error')
		@endslot
		@slot('description')
    ...
    <div class="panel panel-default">
        <div class="panel-heading">
            <div class="panel-title">@lang('centurion::users.headings.assign_permissions', ['first_name' => $user->first_name, 'last_name' => $user->last_name])</div>
        </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="checkbox-list">
                        @foreach($groupedUserPermissions as $abilityCategory => $userPermissions)
                        <div class="checkbox-group">
                            <fieldset>
                                <legend style="font-weight: bold;">{{ $abilityCategory }}</legend>
                                @foreach($userPermissions as $userPermission)
                                    <br />
                                    <span class="tristate tristate-accept-deny-checkbox">
                                        <input type="radio" class="deny-checkbox" id="{{ 'abilityCheckBox' . $userPermission['ability']->id }}-state-off" data-tristate-name="denied_permissions[]" value="{{ $userPermission['ability']->id }}"
                                            @if ($userPermission['allowed'] == '0')
                                                checked="checked"
                                            @endif
                                        />
                                        <input type="radio" class="null-checkbox" id="{{ 'abilityCheckBox' . $userPermission['ability']->id }}-state-null" data-tristate-name="null_permissions[]" value="{{ $userPermission['ability']->id }}"
                                            @if ($userPermission['allowed'] == '')
                                                checked="checked"
                                            @endif
                                        />
                                        <input type="radio" class="accept-checkbox" id="{{ 'abilityCheckBox' . $userPermission['ability']->id }}-state-on" data-tristate-name="accepted_permissions[]" value="{{ $userPermission['ability']->id }}"
                                            @if ($userPermission['allowed'] == '1')
                                                checked="checked"
                                            @endif
                                        />
                                        <i></i>
                                        <label for="{{ 'abilityCheckBox' . $userPermission['ability']->id }}-state-off">{{ $userPermission['ability']->name }}</label>
                                        <label for="{{ 'abilityCheckBox' . $userPermission['ability']->id }}-state-null">{{ $userPermission['ability']->name }}</label>
                                        <label for="{{ 'abilityCheckBox' . $userPermission['ability']->id }}-state-on">{{ $userPermission['ability']->name }}</label>
                                    </span>
                                @endforeach
                            </fieldset>
                        </div>
